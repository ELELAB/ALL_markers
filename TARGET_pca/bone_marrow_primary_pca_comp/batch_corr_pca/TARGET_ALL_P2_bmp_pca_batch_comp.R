...
# Load in gene expression data batch corrected for batch effect year of diagnosis
ALL_P2_bmp_batch_corr <- get(load("TARGET_batch/bone_marrow_primary_batch_comp/TARGET_ALL_P2_bmp_corrected_year.rda"))
...
MDS_subtype <- MDSPlot(my.data = ALL_P2_bmp_batch_corr, 
                       my.group = ALL_P2_info$subtype_abbr, 
                       my.labels = colnames(ALL_P2_bmp_batch_corr), 
                       my.cols = c("orange", "darkgreen"))
ggsave(filename = "ALL_P2_bmp_subtype_MDS_batch.pdf", 
       plot = MDS_subtype, 
       path = "TARGET_pca/bone_marrow_primary_pca_comp/batch_corr_pca/", 
       width = 12, 
       height = 8)

# Create MDS plot using MDSPlot function (from CAMPP pipeline) coloring for year of diagnosis of samples
MDS_year <- MDSPlot(my.data = ALL_P2_bmp_batch_corr, 
                    my.group = ALL_P2_info$diagnosis_year, 
                    my.labels = colnames(ALL_P2_bmp_batch_corr), 
                    my.cols = c("#FFEDA0", "#FED975", "#FEC44F", "#FD8D3C", "#FC4E2A", "#E3201B", "#BD1926", "#7F0E26"))
ggsave(filename = "ALL_P2_bmp_year_MDS_batch.pdf", 
       plot = MDS_year, 
       path = "TARGET_pca/bone_marrow_primary_pca_comp/batch_corr_pca/", 
       width = 12, 
       height = 8)
...
MDS_portion <- MDSPlot(my.data = ALL_P2_bmp_batch_corr, 
                       my.group = ALL_P2_info$tissue_portion_aliquot, 
                       my.labels = colnames(ALL_P2_bmp_batch_corr), 
                       my.cols = c("orange", "darkgreen"))
ggsave(filename = "ALL_P2_bmp_portion_MDS_batch.pdf", 
       plot = MDS_portion, 
       path = "TARGET_pca/bone_marrow_primary_pca_comp/batch_corr_pca/", 
       width = 12, 
       height = 8)
...
MDS_isolation <- MDSPlot(my.data = ALL_P2_bmp_batch_corr, 
                         my.group = ALL_P2_info$number_isolation_aliquot, 
                         my.labels = colnames(ALL_P2_bmp_batch_corr), 
                         my.cols = c("orange", "darkgreen"))
ggsave(filename = "ALL_P2_bmp_isolation_MDS_batch.pdf", 
       plot = MDS_isolation, 
       path = "TARGET_pca/bone_marrow_primary_pca_comp/batch_corr_pca/", 
       width = 12, 
       height = 8)
...
ggsave(filename = "ALL_P2_bmp_subtype_year_MDS_batch.pdf",
       plot = subtype_year, 
       path = "TARGET_pca/bone_marrow_primary_pca_comp/batch_corr_pca/",
       width = 12, 
       height = 8)
...
ggsave(filename = "ALL_P2_bmp_subtype_portion_MDS_batch.pdf",
...
ggsave(filename = "ALL_P2_bmp_subtype_isolation_MDS_batch.pdf",
